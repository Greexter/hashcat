...
static const char *HASH_NAME      = "BitwardenPin";
...
// $bitwardenpin$1*iterations*salt*iv*encrypted_key*mac
static const char *ST_HASH        = "$bitwardenpin$1*100000*amFrdWJzdHJvbUBnbWFpbC5jb20=*2YP1xSbiwu6kWZj71oNhoQ==*T7lFnx2xuHTaSHLeFAAG/TqABb6cG8X0hEL/s1UPbvJJ2JbLXObzyfNq49rCMozg*RNm+yTxP4mskJqxueJ57C4IssE+Ey+a0UgIBBcESw2w=";
...
typedef struct bitwarden_pin_tmp
{
  u32 ipad[8];
  u32 opad[8];
...
typedef struct bitwarden_pin {
  u32 iv[4];
  u32 data[12];
} bitwarden_pin_t;

static const char *SIGNATURE_BITWARDEN = "$bitwardenpin$";
...
{
  const u64 esalt_size = (const u64) sizeof (bitwarden_pin_t);
...
{
  const u64 tmp_size = (const u64) sizeof (bitwarden_pin_tmp_t);
...
int module_hash_decode (MAYBE_UNUSED const hashconfig_t *hashconfig, MAYBE_UNUSED void *digest_buf, MAYBE_UNUSED salt_t *salt, MAYBE_UNUSED void *esalt_buf, MAYBE_UNUSED void *hook_salt_buf, MAYBE_UNUSED hashinfo_t *hash_info, const char *line_buf, MAYBE_UNUSED const int line_len)
{
  ...
  bitwarden_pin_t *bitwarden_pin = (bitwarden_pin_t *) esalt_buf;
  ...
  if (tmp_len != 48) return (PARSER_IV_LENGTH);

  memcpy (bitwarden_pin->data, tmp_buf, tmp_len);
...
int module_hash_encode (MAYBE_UNUSED const hashconfig_t *hashconfig, MAYBE_UNUSED const void *digest_buf, MAYBE_UNUSED const salt_t *salt, MAYBE_UNUSED const void *esalt_buf, MAYBE_UNUSED const void *hook_salt_buf, MAYBE_UNUSED const hashinfo_t *hash_info, char *line_buf, MAYBE_UNUSED const int line_size)
{
  ...
  base64_encode (int_to_base64, (const u8 *) bitwarden_pin->iv, 16, (u8 *) iv_buf);

  // data_buf

  u8 data_buf[100] = { 0 };

  base64_encode (int_to_base64, (const u8 *) bitwarden_pin->data, 48, (u8 *) data_buf);
  ...
  const int line_len = snprintf (line_buf, line_size, "%s1*%i*%s*%s*%s*%s",
    SIGNATURE_BITWARDEN,
    salt->salt_iter + 1,
    salt_buf,
    iv_buf,
