...
KERNEL_FQ void m90000_init (KERN_ATTR_TMPS (bitwarden_pin_tmp_t))
{
  /**
   * base
   */
...
KERNEL_FQ void m90000_loop (KERN_ATTR_TMPS (bitwarden_pin_tmp_t))
{
  const u64 gid = get_global_id (0);
...
KERNEL_FQ void m90000_comp (KERN_ATTR_TMPS_ESALT (bitwarden_pin_tmp_t, bitwarden_pin_t))
{
  /**
   * base
   */
  ...
  // use HKDF to diversify key = using hmac-sha256 with key and "mac".0x01 salt ("enc" for encryption key)
  // 
  ...
  u32 buf[16] = {0};
  buf[0] = sha256_hmac_ctx.opad.h[0];
  buf[1] = sha256_hmac_ctx.opad.h[1];
  buf[2] = sha256_hmac_ctx.opad.h[2];
  buf[3] = sha256_hmac_ctx.opad.h[3];
  buf[4] = sha256_hmac_ctx.opad.h[4];
  buf[5] = sha256_hmac_ctx.opad.h[5];
  buf[6] = sha256_hmac_ctx.opad.h[6];
  buf[7] = sha256_hmac_ctx.opad.h[7];

  sha256_hmac_ctx_t sha256_hmac_ctx2;

  sha256_hmac_init (&sha256_hmac_ctx2, buf, 32);

  sha256_hmac_update_global_swap (&sha256_hmac_ctx2, esalt_bufs[DIGESTS_OFFSET_HOST].iv, 16);
  sha256_hmac_update_global_swap (&sha256_hmac_ctx2, esalt_bufs[DIGESTS_OFFSET_HOST].data, 48);
